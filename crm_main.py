...
# قائمة كافة الدول العربية (للتحديث الجديد)
COUNTRY_CODES = {
...
def init_db():
    ...
    # جدول سجل مدة الاستخدام المضاف للمدير
...
def is_duplicate_company(new_name):
    """التحقق الذكي من تكرار الاسم عبر مقارنة الكلمات الأساسية"""
    ...
    def clean(n):
        n = n.lower().strip()
        for word in stop_words: n = n.replace(word, "")
        return set(re.findall(r'\w+', n))
    new_tokens = clean(new_name)
    for existing_name in existing:
        existing_tokens = clean(existing_name)
...
if not st.session_state['logged_in']:
    ...
    with col2:
        ...
        if st.button("تأكيد"):
            if choice == "تسجيل دخول":
                account = login_user(user_input, pw_input)
                if account:
                    st.session_state.update({'logged_in': True, 'user_role': account[2], 'real_name': account[3], 'username': user_input, 'login_time': datetime.now()})
                    conn.execute("INSERT INTO user_sessions (username, login_time) VALUES (?,?)", (user_input, datetime.now().strftime("%Y-%m-%d %H:%M:%S")))
                    conn.commit()
                    st.rerun()
                else: st.error("بيانات خاطئة")
            else:
                if create_user(user_input, pw_input, user_input): st.success("تم التسجيل")
    ...
    if nav == "بوابة المبيعات":
        st.header("💼 بوابة المتابعة والإنجاز")
        my_data = get_my_data(st.session_state['real_name'])
        if not my_data.empty:
            search_box = st.text_input("🔎 ابحث في عملائك (اسم، جوال...):")
            df_filtered = my_data[my_data.astype(str).apply(lambda x: x.str.contains(search_box, case=False)).any(axis=1)] if search_box else my_data
            ...
            if client_options:
                ...
                with c1:
                    ...
                    st.link_button("💬 مراسلة واتساب فورية", f"https://wa.me/{wa_number}", use_container_width=True)
                with c2:
                    ...
                    with st.form("status_up"):
                        ...
                        if st.form_submit_button("✅ حفظ"):
                            update_customer_status(sid, client_row['company_name'], new_stage, st.session_state['real_name'], note)
                            st.success("تم التحديث"); st.rerun()

    elif nav == "إضافة عميل":
        st.header("➕ إضافة عميل جديد")
        with st.form("new_intl_c"):
            c1, c2 = st.columns(2)
            with c1:
            ...
            if st.form_submit_button("حفظ"):
                ...
                is_valid_mob, formatted_mob = validate_intl_mobile(COUNTRY_CODES[ckey], mob_in)
                full_mobile = f"+{COUNTRY_CODES[ckey]}{formatted_mob}"
                
                if is_dup: st.error(f"❌ العميل مسجل مسبقاً باسم مشابه: ({dup_name})")
                elif not comp or not is_valid_mob or not validate_email(em):
                    st.error("يرجى التأكد من ملء الحقول وصحة رقم الجوال لهذه الدولة")
                else:
                    conn.execute("INSERT INTO customers (company_name, sector, contact_person, position, mobile, email, sales_rep) VALUES (?,?,?,?,?,?)",
                                 (comp, sec, cont, pos, full_mobile, em, st.session_state['real_name']))
                    conn.commit(); st.success(f"تم الحفظ بنجاح: {full_mobile}")

    elif nav == "لوحة المدير" and role == 'admin':
        st.header("📊 لوحة المدير الشاملة")
        t1, t2 = st.tabs(["الإحصائيات", "⏱️ سجل استخدام المناديب"])
        with t1:
            df_all = get_all_data()
            if not df_all.empty:
                st.plotly_chart(px.bar(df_all, x='sales_rep', color='status', title="توزيع العملاء على المناديب"))
        with t2:
            st.subheader("تقرير مدة بقاء الموظفين على البرنامج")
            sessions = pd.read_sql("SELECT username as 'الموظف', login_time as 'الدخول', logout_time as 'الخروج', duration_mins as 'المدة (دقيقة)' FROM user_sessions ORDER BY id DESC", conn)
            st.dataframe(sessions, use_container_width=True)

    # بقية الخيارات (استيراد، مستخدمين، بحث) تظل كما في كودك المعتمد
