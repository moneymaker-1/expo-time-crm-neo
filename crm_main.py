...
# --- قاعدة البيانات (نفس هيكلك الأصلي تماماً) ---
def init_db():
...
if not st.session_state['logged_in']:
    ...
    # --- بوابة المبيعات (استعادة التكملة الذكية والملاحظات) ---
    if nav == "بوابة المبيعات":
        ...
        with tab_my:
            ...
            if not my_data.empty:
                # ميزة التكملة الذكية (البحث اللحظي)
                search_q = st.text_input("🔎 ابحث بالاسم (التكملة الذكية مفعلة):")
                df_view = my_data[my_data['company_name'].str.contains(search_q, case=False)]
                
                # ربط الاختيار بنتائج البحث (التكملة الذكية)
                client_options = {row['id']: row['company_name'] for i, row in df_view.iterrows()}
                if client_options:
                    sid = st.selectbox("👇 اختر العميل من الاقتراحات:", list(client_options.keys()), format_func=lambda x: client_options[x])
                    ...
                    with st.form("edit_form"):
                        ...
                        with c2:
                            ...
                            # خانة مبلغ التعميد
                            amt = st.number_input("مبلغ التعميد (SAR)", value=float(row['contract_amount'])) if new_st == "تم التعميد" else 0.0
                            # استعادة الملاحظات الأصلية
    ...
    # --- استعادة قسم المستخدمين الأصلي بالكامل ---
    elif nav == "المستخدمين" and role == 'admin':
        st.header("👥 إدارة المستخدمين (الأصلية)")
        t1, t2, t3 = st.tabs(["إضافة مستخدم", "تعديل كلمة مرور", "حذف مستخدم"])
        
        with t1: # إضافة
            n = st.text_input("الاسم الكامل")
            u = st.text_input("اسم المستخدم (اليوزر)")
            p = st.text_input("كلمة المرور (الباسورد)")
            if st.button("إنشاء حساب"):
                try:
                    conn.execute("INSERT INTO users VALUES (?, ?, ?, ?)", (u, p, 'rep', n))
                    conn.commit(); st.success("تمت الإضافة")
                except: st.error("اليوزر موجود مسبقاً")
        
        users_list = pd.read_sql("SELECT username FROM users", conn)['username'].tolist()
        with t2: # تعديل الباسورد
            u_sel = st.selectbox("اختر المستخدم للتعديل", users_list)
        ...
        with t3: # حذف
            u_del = st.selectbox("اختر المستخدم للحذف", [x for x in users_list if x != 'admin'])
    ...
    # --- استعادة قاعدة البيانات الشاملة والاستيراد والبحث ---
        ...
        st.dataframe(pd.read_sql("SELECT username as 'الموظف', login_time, duration_mins FROM user_sessions", conn), use_container_width=True)
