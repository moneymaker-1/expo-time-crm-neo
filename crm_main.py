...
# تحسين مظهر الجداول والقوائم عبر CSS
st.markdown("""
    <style>
    .stButton>button { width: 100%; border-radius: 5px; height: 3em; background-color: #007bff; color: white; }
    .stContainer { border: 1px solid #e6e9ef; padding: 20px; border-radius: 10px; background-color: white; }
    </style>
...
if not st.session_state['logged_in']:
    ...
    # --- بوابة المبيعات (تم تصحيح المسمى والتشغيل) ---
    if nav == "بوابة المبيعات":
        st.header("💼 إدارة علاقات العملاء")
        rep_n = st.session_state['real_name']
        if role == 'admin':
            reps_df = pd.read_sql("SELECT real_name FROM users WHERE role='rep'", conn)
            reps = reps_df['real_name'].tolist() if not reps_df.empty else []
            rep_n = st.selectbox("اختر المندوب للعرض:", ["الكل"] + reps) if reps else rep_n
        ...
        if not my_data.empty:
            ...
            with col_info:
                with st.container():
                    ...
                    st.markdown(f"**📍 القطاع:** {row['sector'] or 'غير محدد'}")

            with col_action:
                with st.form("status_update_form"):
                    st.markdown("#### 🚀 تحديث مرحلة العميل")
                    current_idx = TRIP_STAGES.index(row['status']) if row['status'] in TRIP_STAGES else 0
                    new_st = st.selectbox("انقل العميل إلى:", TRIP_STAGES, index=current_idx)
                    note = st.text_area("سجل ملاحظات المتابعة")
    ...
    # --- إضافة عميل ---
        ...
        with st.form("add_client_form", clear_on_submit=True):
            ...
            if st.form_submit_button("💾 حفظ العميل في النظام"):
                ...
                if exists: 
                    st.error(f"⚠️ العميل مسجل مسبقاً مع المندوب: {exists[0]}")
                elif comp and mob:
    ...
    # --- إدارة المستخدمين ---
        ...
        with t1:
            with st.form("new_user"):
                ...
                if st.form_submit_button("إنشاء حساب مندوب"):
                    conn.execute("INSERT INTO users VALUES (?,?,?,?)", (u, p, 'rep', n)); conn.commit(); st.success("تم الإنشاء"); st.rerun()
            st.divider()
    ...
    # --- استيراد ملف ---
        ...
        if f and st.button("بدء عملية الاستيراد"):
            ...
            st.success("✅ تم استيراد العملاء بنجاح")
