...
if not st.session_state['logged_in']:
    ...
    # --- بوابة المبيعات (الشكل الأصلي + الإضافات) ---
    if nav == "بوابة Mبيعات":
        ...
        # إضافة قسم التنبيهات في الأعلى
        if not my_data.empty:
            for _, row in my_data.iterrows():
                if row['status'] == "تم تقديم عرض مالي" and row['quote_date']:
                    if (datetime.now() - datetime.strptime(row['quote_date'], "%Y-%m-%d")).days >= 3:
                        st.error(f"⚠️ تنبيه: عرض العميل **{row['company_name']}** ينتهي اليوم (مر 3 أيام)")

        if not my_data.empty:
            client_options = {row['id']: f"{row['company_name']} - {row['contact_person']}" for i, row in my_data.iterrows()}
            ...
            with c2:
                ...
                with st.form("status_update"):
                    new_stage = st.selectbox("انقل العميل للمرحلة التالية:", TRIP_STAGES, index=TRIP_STAGES.index(client_row['status']))
    ...
    # --- إضافة عميل (الشكل الأصلي الكامل) ---
    elif nav == "إضافة عميل":
    ...
    # --- لوحة المدير (الإحصائيات) ---
        ...
        if not all_df.empty:
            st.metric("إجمالي العروض", len(all_df))
            st.metric("إجمالي قيم التعميد", f"{all_df['contract_value'].sum()} ريال")
            st.dataframe(all_df)
